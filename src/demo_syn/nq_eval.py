...
flags.DEFINE_string(
    'gold_path', './nq-dev-sample.jsonl.gz', 'Path to the gzip JSON data. For '
    'multiple files, should be a glob '
    'pattern (e.g. "/path/to/files-*"')
flags.DEFINE_string('predictions_path', './predictions_sample.json', 'Path to prediction JSON.')
flags.DEFINE_bool(
...
def main(_):
  ...
  if FLAGS.pretty_print:
    ...
    print('*' * 20)
    print('NUMBER OF VALIDATION SAMPLES (n={}):'.format(scores['long-answer-n']))
    print('              F1     /  P      /  R')
